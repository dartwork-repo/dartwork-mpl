...
x = dm.np.linspace(0, 10, 200)
ax.plot(x, dm.np.sin(x), color="dm.blue5", label="signal")
...
for ax, label in zip(axes, "ABCD"):
    ax.plot(dm.np.linspace(0, 1, 40), dm.np.random.rand(40), color="dm.blue6", lw=0.8)
...
ax.plot(dm.np.arange(50), dm.np.cumsum(dm.np.random.randn(50)) + 20, color="dm.blue6")
dm.simple_layout(fig)
