# 코딩 에이전트 규칙 (Coding Agent Rules)

이 문서는 dartwork-mpl 프로젝트에서 코딩 에이전트가 따라야 할 규칙들을 정의합니다. 각 규칙은 예시, 설명, 그리고 사용해야 하는 이유를 포함합니다.
...
### 규칙
- **라이브러리 설치**: `uv add <package-name>`
- **스크립트 실행**: `uv run <script>`
- **도구 실행**: `uvx <tool-name> <args>` (예: `uvx ruff check`)
...
# 스크립트 실행
uv run python src/dartwork_mpl/cli.py
...
### 설명
`uv`는 Rust로 작성된 빠른 Python 패키지 관리자입니다. pip와 비교하여:
- **속도**: pip보다 10-100배 빠른 설치 속도
- **의존성 해결**: 더 정확하고 빠른 의존성 해결
- **통합**: 프로젝트 관리, 가상환경, 패키지 설치를 하나의 도구로 통합

### 사용해야 하는 이유
1. **성능**: 대규모 프로젝트에서 의존성 설치 시간을 크게 단축
2. **일관성**: 모든 팀원이 동일한 패키지 관리 도구를 사용하여 환경 일관성 보장
3. **표준화**: `uvx`를 통해 프로젝트별 도구 설치 없이 일회성 도구 실행 가능
...
def plot_data(data: list[float]) -> None:
    import matplotlib.pyplot as plt  # 함수 호출 시에만 로드
    plt.plot(data)
    plt.show()
```
